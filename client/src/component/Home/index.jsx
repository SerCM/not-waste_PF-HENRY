      ...
      {errorMessage || sellersWithPosts[0]?.products?.length === 0 ? (
        <div className="mx-4">
          ...
          <Message
            ...
            message="No se encontraron proveedores/productos con los filtros aplicados"
            type="error"
          />
        </div>
      ) : (
            ...
            <div className="accordion-item">
              ...
              >
                <div className="accordion-body">
                  {sellersWithPosts.length ? (
                    sellersWithPosts.slice(0, 3).map((seller) => {
            ...
            {sellersWithPosts.slice(3, 6).length && (
              <>
                <div className="accordion-item">
                  ...
                  >
                    <div className="accordion-body">
                      {sellersWithPosts.length > 3 &&
                        sellersWithPosts.slice(3, 6).map((seller) => {
                          let postsSeller = seller?.products
                            ?.filter((product) => product?.posts)
                            .map((p) => p.posts)
                            .flat()
                  ...
                  <div className="accordion-item">
                    ...
                    >
                      <button
                        ...
                        type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#panelsStayOpen-collapseTwo123"
                        aria-expanded="false"
                        aria-controls="panelsStayOpen-collapseTwo"
                      >
                        Mostrar todos
                    ...
                    <div
                      id="panelsStayOpen-collapseTwo123"
                      className="accordion-collapse collapse"
                      aria-labelledby="panelsStayOpen-headingTwo"
                    >
                      <div className="accordion-body">
                        {sellersWithPosts.length > 6 &&
                          sellersWithPosts.slice(6).map((seller) => {
                            let postsSeller = seller?.products
                              ?.filter((product) => product?.posts)
                              .map((p) => p.posts)
                              .flat()
